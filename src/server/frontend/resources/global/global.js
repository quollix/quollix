window.showSnackbar = function (msg) {
    ...
    snackbar.setAttribute('data-visible', 'true')

    const text = document.createElement('span')
    text.textContent = msg || 'An error occurred'

    const closeButton = document.createElement('button')
    closeButton.type = 'button'
    closeButton.className = 'snackbar-close lean'
    closeButton.title = 'Close'
    closeButton.setAttribute('aria-label', 'Close notification')
    closeButton.innerHTML = '<i class="mdi mdi-close"></i>'

    const remove = () => snackbar.remove()
    const timeout = setTimeout(remove, 10000)

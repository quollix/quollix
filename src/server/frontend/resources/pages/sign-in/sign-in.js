window.signIn = async (event, username, password) => {
    ...
    const ok = await apiPost('{{ $.Static.Paths.BackendSignIn }}', { username, password })
    if (ok) {
        ...
        window.location.href = nextUrl || '{{ $.Static.Paths.FrontendInstalledApps }}'
    }
}

window.startOidcSignIn = async (providerId) => {
    const [body, ok] = await apiPostWithBody('{{ $.Static.Paths.BackendOidcSignIn }}', { value: providerId })

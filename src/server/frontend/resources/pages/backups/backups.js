window.restoreBackup = async (backupId, maintainer, app, version, backupCreationDate) => {
  const confirmed = await confirmDialog(`Restore backup for ${maintainer}/${app} version ${version} from ${backupCreationDate}? All the current data of this app will be overwritten.`)
  if (!confirmed) return
  ...
  if (app === 'postgres') {
    ...
    if (ok) {
      // Restoring an postgres backup usually invalidates the current admin session cookie, so he need to be redirected to the login page. Also, the restore operation aborts prematurely due to the NETWORK_CHANGED error, so we have to add a delay before redirecting.
      showSnackbar('Backup restored successfully. Redirecting to login…')

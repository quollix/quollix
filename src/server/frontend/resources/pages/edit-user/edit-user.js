window.changeUsername = async (userId, newUsername) => {
    ...
    if (!confirmed) return
    const ok = await apiPost('{{ $.Static.Paths.BackendUsersChangeUsername }}', { user_id: userId, username: newUsername })
...
window.changeEmail = async (userId, newEmail) => {
    ...
    if (!confirmed) return
    const ok = await apiPost('{{ $.Static.Paths.BackendUsersChangeEmail }}', { user_id: userId, new_email: newEmail })

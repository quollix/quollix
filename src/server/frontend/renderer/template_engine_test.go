...
func TestTemplateEngineImpl_CompileText_DoesNotEscapeInScriptContext(t *testing.T) {
	templateText := `<script>{{ with . }}const loginPath = "{{ .Paths.FrontendLogin }}"; const lessThan = "<";{{ end }}</script>`
	outputBytes, err := templateEngine.CompileText("sample", templateText)
	assert.Nil(t, err)
	assert.Equal(t, `<script>const loginPath = "/login"; const lessThan = "<";</script>`, string(outputBytes))
}

...
func TestDockerService_AttachBrandAppToNetwork(t *testing.T) {
	...
	containerInspectOutput, err := exec.Command("docker", "inspect", tools.BrandAppContainerName).CombinedOutput()
	assert.Nil(t, err)
	...
	containerInspectOutputAfterDisconnect, err := exec.Command("docker", "inspect", tools.BrandAppContainerName).CombinedOutput()
	assert.Nil(t, err)

...
func TestRenderContractPage_UsesProductionFrontendResources(t *testing.T) {
	...
	assert.True(t, strings.Index(rendered, pageCssPath) < strings.Index(rendered, `<body>`))
	assert.True(t, strings.Index(rendered, pageJsPath) < strings.Index(rendered, `<body>`))
...
func createFrontendRenderContractTestObjects(t *testing.T) frontendRenderContractTestObjects {
	...
	templateEngine := &renderer.TemplateEngineImpl{}

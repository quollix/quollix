...
func TestOIDC_HappyPath_AuthorizationCodeFlowWithUserinfo(t *testing.T) {
	...
	expectedSub := strconv.Itoa(GetRequiredUserByUsername(t, client, tools.DefaultAdminName).Id)
...
func TestOIDC_EmailClaimUsesSyntheticEmailUnlessRealEmailExposureIsEnabled(t *testing.T) {
	client := GetClientAndLogin(t)
	defer client.Test.ResetTestState()
	InviteUserAndSetPassword(t, client, SampleUsername, SampleUserPassword, SampleRealUserEmail)

	app, err := InstallSample(t, client, "2.0")
	assert.Nil(t, err)

	exposeRealEmail, err := client.Email.ReadExposeRealEmailInOidcToken()
	assert.Nil(t, err)
	assert.False(t, exposeRealEmail)
	assertOidcEmailClaim(t, app, SampleUserEmail)

	assert.Nil(t, client.Email.SaveExposeRealEmailInOidcToken(true))
	assertOidcEmailClaim(t, app, SampleRealUserEmail)

	assert.Nil(t, client.Email.SaveExposeRealEmailInOidcToken(false))
	assertOidcEmailClaim(t, app, SampleUserEmail)
}
...
func assertOidcEmailClaim(t *testing.T, app *apps_basic.AppDto, expectedEmail string) {
	...
	uinfo := ctx.FetchUserinfo(tokens.AccessToken)
	assert.Equal(t, expectedEmail, uinfo.Email)
}

func (c *OidcTestClient) refreshTokens(refreshToken, clientId, clientSecret string, clientAuthMethod ClientAuthMethod) oidc_provider.TokenResponse {
...
func (c *OidcTestClient) refreshTokensRaw(refreshToken, clientId, clientSecret string, clientAuthMethod ClientAuthMethod) *http.Response {
	form := url.Values{}

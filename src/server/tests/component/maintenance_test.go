...
func TestAppAutoMaintenanceSettingsChanges(t *testing.T) {
	...
	client.Apps.UpdateMaintenanceSettings(sampleApp.AppId, true, false)

	sampleApp = client.Apps.GetInstalledSample()
	assert.True(t, sampleApp.AutomaticUpdatesEnabled)
	assert.False(t, sampleApp.AutomaticBackupsEnabled)

	client.Apps.UpdateMaintenanceSettings(sampleApp.AppId, false, false)
...
func TestReadingAndSavingRetentionPolicy(t *testing.T) {
	...
	policyFromBackend := client.Maintenance.ReadRetentionPolicy()
...
func assertBackupCount(t *testing.T, client *QuollixClient, maintainer string, appName string, expectedCount int) []tools.BackupInfo {

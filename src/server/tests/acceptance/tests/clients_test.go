...
func TestClientsPageClientCrud(t *testing.T) {
	...
	page.
		...
		AssertClientSecretMasked(client.Name)
	createdClient := component.GetOnlyOidcRelyingParty(t, frame.Client)
	createdClientFromPage := page.GetRequiredClient(client.Name)
	assert.Equal(t, createdClient.Id, createdClientFromPage.Id)
	assert.Equal(t, createdClient.Name, createdClientFromPage.Name)
	assert.Equal(t, createdClient.Domain, createdClientFromPage.Domain)
	assert.Equal(t, createdClient.ClientId, createdClientFromPage.ClientId)

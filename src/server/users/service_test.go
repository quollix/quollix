...
func TestGetUserIdAndRoleFromQuollixRequest_DisabledUserReturnsAnonymousUser(t *testing.T) {
	testObjects := getUserServiceTestObjects(t)
	request := newRequestWithAuthCookie()
	hashedCookie := testObjects.Service.AuthHelper.GetSHA256Hash(requestAuthCookieValue)
	session := getAuthenticatedSession(7, false, time.Now().Add(time.Hour))
	session.User.IsEnabled = false
	testObjects.SessionRepo.EXPECT().
		GetAuthenticatedSession(hashedCookie, QuollixSessionAudience()).
		Return(session, nil)
...
func TestChangeUsername_WhenReservedEmailTargetExistsReturnsSpecificError(t *testing.T) {
	...
	testObjects.UserRepo.EXPECT().GetUserById(42).Return(user, nil)
	testObjects.UserRepo.EXPECT().DoesUserExist("thomas").Return(false, nil)
	testObjects.UserRepo.EXPECT().DoesEmailExist("thomas@example.invalid").Return(true, nil)

	err := testObjects.Service.ChangeUsername(42, "thomas")
	assert.Equal(t, ReservedEmailRenameConflictError, u.ExtractError(err))
}

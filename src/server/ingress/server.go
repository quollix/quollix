...
func (s *ServerListener) OpenPortsAndListen() ([]*http.Server, error) {
	...
	httpHandler := u.RecoverHttpPanics(s.buildHttpHandler(authHandler))
	tlsHandler := u.RecoverHttpPanics(authHandler)
	...
	tlsServer, err := s.buildTlsServer(tlsHandler)
	if err != nil {
		return nil, err
	}
...
func (s *ServerListener) buildHttpHandler(next http.Handler) http.Handler {
	if s.Config.RedirectHttpToHttps {
		return redirectingMiddleware.Handler(next)
	}
	return next
}

...
func (a *AuthServiceImpl) IsRequestAddressedToAnApp(requestHost, originHeaderValue string) (bool, error) {
	databaseHost, err := a.ConfigsService.GetServerHost()
	if err != nil {
		return false, err
	}
	...
	err = a.AppRequestPolicy.ValidateRequestOrigin(requestHost, requestOriginHost, databaseHost)
	if err != nil {
		return false, err
	}

	return a.AppRequestPolicy.IsRequestAddressedToAnApp(requestHost, databaseHost), nil
}

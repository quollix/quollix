...
func headerHandler(w http.ResponseWriter, r *http.Request) {
	requestedHeaderName := strings.TrimPrefix(r.URL.Path, "/header/")
	if requestedHeaderName == "" {
		http.Error(w, "unknown header", http.StatusBadRequest)
		return
	}

	if _, err := w.Write([]byte(r.Header.Get(requestedHeaderName))); err != nil {
		http.Error(w, "error writing response", http.StatusInternalServerError)
		log.Printf("Error writing response: %v", err)
	}
}
...
func saveStringHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = os.WriteFile("/data/string.txt", data, 0o600)
	if err != nil {

...
func TestSaveMaintenanceConfig_InvalidHourTooLow(t *testing.T) {
	testObjects := newMaintenanceServiceTestObjects(t)
	defer assertMaintenanceServiceExpectations(t, testObjects)
	err := testObjects.Service.SaveMaintenanceConfig("Europe/Berlin", -1)
	deepstack.AssertDeepStackError(t, err, InvalidMaintenanceWindowStartHourErrorMessage)
}

func TestSaveMaintenanceConfig_InvalidHourTooHigh(t *testing.T) {
	testObjects := newMaintenanceServiceTestObjects(t)
	defer assertMaintenanceServiceExpectations(t, testObjects)
	err := testObjects.Service.SaveMaintenanceConfig("Europe/Berlin", 24)
	deepstack.AssertDeepStackError(t, err, InvalidMaintenanceWindowStartHourErrorMessage)
}

func TestSaveMaintenanceConfig_InvalidTimezone(t *testing.T) {
	testObjects := newMaintenanceServiceTestObjects(t)
	defer assertMaintenanceServiceExpectations(t, testObjects)
	timezone := "Not/A_Timezone"
	testObjects.TimezoneProvider.EXPECT().IsIanaTimezoneValid(timezone).Return(false)
	err := testObjects.Service.SaveMaintenanceConfig(timezone, 2)
	deepstack.AssertDeepStackError(t, err, InvalidIanaTimezoneErrorMessage)
}

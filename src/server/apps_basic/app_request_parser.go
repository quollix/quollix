...
type AppRequestParser interface {
	...
	GetAppNameFromRequestHost(requestHost, baseDomain string) (string, error)
	GetQuerySecret(r *http.Request) (string, bool, bool)
}

const HostDoesNotEndWithBaseDomainError = "host in HTTP request does not end with base domain, but it should have"
...
func (a *AppRequestParserImpl) GetAppNameFromRequestHost(requestHost, baseDomain string) (string, error) {
	if !strings.HasSuffix(requestHost, baseDomain) {
		return "", u.Logger.NewError(HostDoesNotEndWithBaseDomainError, tools.RequestHostField, requestHost, tools.BaseDomainField, baseDomain)
	}

	return strings.TrimSuffix(requestHost, "."+baseDomain), nil
}

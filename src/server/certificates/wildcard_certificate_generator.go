...
const CertificatChallengeNotPossibleForLocalhost = "certificate challenge is not possible for server host 'localhost'"
...
func (w *WildcardCertificateServiceImpl) StartDns01Session() (*Dns01Session, *Dns01ChallengeInfo, error) {
	host, err := w.ConfigsRepository.GetConfig(configs.ConfigKeys.ServerHost)
	if err != nil {
		return nil, nil, err
	}

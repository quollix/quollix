...
func GetRunningContainers() []string {
	output := Tr.Cmd().AllowFail().Quiet().Run("docker ps -a --format '{{.Names}}'").Output()
	return strings.Fields(output)
}

func getContainersWithLabel(label string) []string {
	output := Tr.Cmd().AllowFail().Quiet().Run("docker ps -aq --filter label=%s", label).Output()
	return strings.Fields(output)
}

func CustomCleanup() {
	for _, container := range projectCleanupContainers {
		Tr.Cmd().AllowFail().Quiet().Run("docker rm -f %s", container)
	}
	...
	for _, volume := range projectCleanupVolumes {
		Tr.Cmd().AllowFail().Quiet().Run("docker volume rm -f %s", volume)
	}

	for _, network := range projectCleanupNetworks {
		Tr.Cmd().AllowFail().Quiet().Run("docker network rm %s", network)
	}
}
